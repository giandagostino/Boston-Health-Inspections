...
airport <- east_boston |>
  filter(address_full == "100   LOGAN AIRPORT TRMNL A")
...
-   Categories withineach designation must be mutually exclusive.
...
model <- glm(viol_status ~ n_neighbor, data = new_bos_insp, family = binomial)


grid <- tibble(
  n_neighbor = seq(
    min(new_bos_insp$n_neighbor),
    max(new_bos_insp$n_neighbor),
    length.out = 300
  ))
...
grid <- grid |>
  mutate(
    pred_prob = predict(model, newdata = grid, type = "response")
  )

  
ggplot() +
  geom_jitter(data = new_bos_insp,
              aes(x = n_neighbor, y = viol_status_num),
              height = 0.1, alpha = 0.2) +
  geom_line(data = grid,
            aes(x = n_neighbor, y = pred_prob),
            linewidth = 1.2)
